...
- [ ] Searching for a document category returns the artifact that satisfies it, with a passage, and returns zero clients
- [ ] A document scores as its best chunk, and the passage returned is that chunk's span of the content
- [ ] Full document content never leaves the database for a search; results omit it and carry the owning client's name instead
- [ ] A query matching both types returns the client first, even when the document's similarity is numerically higher
- [ ] Chunks written under a different model identifier are invisible to search, and the document returns no semantic hit rather than a wrong one
- [ ] A document created moments earlier is found with no wait and no retry
- [ ] The two retrievers run concurrently, and either one failing returns a server error rather than a partial list
- [ ] Documents below the semantic floor are excluded
