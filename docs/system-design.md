...
- **Ordering** is deterministic by type: each retriever applies its own floor, then clients rank above documents (PRD §5.4). No fusion — the corpora are disjoint, so there is no evidence to fuse.
...
| Stored and query vectors come from the same model | `embedding_model` written at insert, filtered at query time (§6.3). A mismatch returns nothing rather than nonsense |
...
- `score` is the **retriever's own score**, rounded to 6 decimals — `word_similarity` for clients, cosine for documents. It is comparable *within* a type and meaningless across types, which is exactly why ordering is by type and not by score (§6.4, PRD §5.5). The example above shows a client at `0.833` and a document at `0.412`; the client is first because it is a client, not because `0.833 > 0.412`.
...
Because ordering is a concatenation (§6.4), paging is a plain slice of `clients ++ documents` — deep pages are stable and need no re-ranking.
...
flowchart LR
    ...
    LR --> F["ResultOrdering<br/>clients ++ documents"]
    SR --> F
...
The lexical SQL runs on one virtual thread while the other embeds the query and then runs the vector SQL. The two branches share nothing and are joined with `CompletableFuture` on a virtual-thread executor. `StructuredTaskScope` would be the cleaner fit, but it is still a preview API in Java 25.
...
- `lexicalFloor` is **fixed at 0.6**, pg_trgm's own default `word_similarity_threshold`, and is *guarded* by the eval set rather than derived from it (§12.3). **It is the riskiest number in the system** (PRD §5.2): because clients are placed above all documents (§6.4), too low a floor fails J2 by promoting a weak client above the utility bill, and too high a floor fails J1. The verified table above is the J1 side; §12.3's client-absence assertions are the J2 side. Note `passport` → `john.doe@neviswealth.com` scores `0.000` — that separation is the property J2 relies on, and 0.6 sits comfortably inside it.
...
**Known limits:** transpositions in short names (`Jhon`) and one- or two-character queries score poorly. `fuzzystrmatch` Levenshtein on names is the follow-up if the demo shows it matters.

### 6.3 Semantic retriever (documents)
...
`ResultOrdering` is a pure static function over the two already-filtered, already-sorted lists:

```
order = clients (by word_similarity DESC, last_name, id)
     ++ documents (by cosine DESC, document_id)
```

That is the whole implementation. There is no fusion function, no `k`, and no score reconciliation — PRD §5.4 explains why: the two retrievers cover **disjoint corpora**, so no item ever appears in both lists and there is no evidence to combine. Reciprocal Rank Fusion over disjoint inputs degenerates into round-robin interleaving with a tie at every position, which would have made an unspecified tie-break the real ranking rule while carrying a name that implies something more principled.

Three consequences worth stating plainly, because they are where this design can go wrong:

1. **The floors carry *all* of the cross-type relevance judgement.** Nothing downstream can rescue a bad floor: a client that clears `lexicalFloor` is placed above every document, however good those documents are. This is why §12.3 tunes both floors against positives *and* negatives and fails the build when the gap closes.
...
3. **Ordering is total and deterministic**, so it is unit-testable with plain lists and no fixture corpus, and pagination is a slice rather than a re-rank (§4.4).
...
The passage is extracted in SQL, so full content never leaves the database for search. Offsets are code points in both Java (`codePointCount`) and Postgres (`substr` counts characters), so they agree even for non-BMP text. Results are re-ordered in memory to match the order from §6.4.
...
The relevance floors (`search.lexical-floor`, `search.semantic-floor`) and fetch depth live in `application.yaml` as tuned constants. They are not deployment knobs, although Spring's relaxed binding allows overriding them during tuning.
...
- `ResultOrdering`: clients precede documents regardless of relative score, within-type order preserved, either list empty, both empty, slicing beyond total.
...
| **J2** | `GET /search?q=address proof` → the utility-bill document is in the results, above the floor, with a passage — **and zero client results**, since any client would outrank it (§6.4) |
...
| Model guard | A chunk written under a different `embedding_model` is invisible to search; the document returns no semantic hit rather than a wrong one (guards the `embedding_model` filter in §6.3) |
...
| Ordering + slice | < 1 ms | Concatenation, not fusion (§6.4) |
...
|---|---|
| Semantic SQL p95 > ~100 ms | HNSW index + top-K rewrite (§6.3) |
...
| Result quality plateaus | Cross-encoder rerank of the top-50 (PRD §8.3); a document-title lexical retriever (§6.4) — at which point documents have two signals and RRF becomes genuinely useful |
